...
for app in ${NEXTCLOUD_APPS:-}; do
	if ! php occ app:enable "$app" 2>&1; then
		php occ app:install "$app" 2>&1 || {
			echo "Warning: could not install/enable ${app}"
			FAILED_APPS+=("$app")
		}
	fi
done
...
if [ -n "${COLLABORA_URL:-}" ] && ! echo "${COLLABORA_URL}" | grep -q "yourdomain.com"; then
	...
	# Set the URL Collabora uses to reach Nextcloud. Without this, a value carried
	# over from a previous installation (e.g. http://nextcloud-aio-apache:23973 from
	# Nextcloud AIO) persists in the database and breaks document editing.
